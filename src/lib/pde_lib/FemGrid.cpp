...
FemGrid::FemGrid(size_t dim, std::vector<double> &vertices, std::vector<IFiniteElement*> &elements, std::vector<IBoundaryElement*> &boundary_elements) : elements(elements),
	vertices(vertices),
...
double FemGrid::norm2(const std::vector<double> &difference)
{
	...
	for (IFiniteElement* element : elements)
	{
		...
		{
			sum += element->get_lumped(i) * difference[element->global_indices[i]] * difference[element->global_indices[i]];  // / elements[i].det_j
		}
	}
